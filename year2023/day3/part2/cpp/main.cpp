...
long GetPower(const game_t &game) {
    long max_red = 0;
    long max_green = 0;
    long max_blue = 0;

    for(const auto& move : game.moves) {
        max_red = std::max(move.red, max_red);
        max_green = std::max(move.green, max_green);
        max_blue = std::max(move.blue, max_blue);
    }

    return max_red*max_green*max_blue;
}

long GetSolution(const std::vector<game_t> &games) {
    long sum = 0;
