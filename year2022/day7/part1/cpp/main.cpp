...
public:
  file_node_t() = default;
  file_node_t(const std::string &name, std::size_t size)
      : _name(name), _size(size) {}
  virtual ~file_node_t() = default;

  [[nodiscard]] virtual const std::string &getName() const { return _name; }

  virtual void setName(const std::string &name) { _name = name; }

  [[nodiscard]] virtual size_t getSize() const { return _size; }
...
public:
  ...
  directory_t(const std::string &name, std::size_t size, directory_t *parent)
      : file_node_t(name, size), _parent(parent) {}

  [[nodiscard]] directory_t *getParent() { return _parent; }

  void setParent(directory_t *parent) { _parent = parent; }
  ...
  [[nodiscard]] std::size_t getSize() const override {
    ...
    auto result =
        std::accumulate(_includes.cbegin(), _includes.cend(), 0, sum_func);
  ...
  void
  get_all_directories(std::vector<const directory_t *> &directories) const {
...
public:
  ...
  [[nodiscard]] directory_t *getRootDir() const {
    return _root_directory.get();
  }
};
...
public:
  device() { _current_dir = _file_system.getRootDir(); }
  ...
  getDirectorySmallerEqualThen(const std::size_t at_most) const {
...
private:
  void execute_change_dir_command(const std::string &cd_command) {
    ...
    const auto current_dir =
        _current_dir->getDirectoryByName(change_to_dir_name);
  ...
  void execute_list_command(std::size_t &line_number,
                            const std::vector<std::string> &programm) {
  ...
  void execute_command(std::size_t &line_number,
                       const std::vector<std::string> &programm) {
    if (programm.empty()) {
      return;
    }
