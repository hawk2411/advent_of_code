...
class file_system_t {
    const std::size_t  TOTAL_DISC_SPACE = 70000000;
...
public:
    ...
    [[nodiscard]] std::size_t getUsedSpace() const {return  _root_directory->getSize(); }
    [[nodiscard]] std::size_t getFreeSpace() const {return TOTAL_DISC_SPACE - getUsedSpace(); }
};
...
public:
    ...
    [[nodiscard]] std::vector<const directory_t *> getDirectorySmallerEqualThen(const std::size_t at_most) const {
        ...
        std::vector<const directory_t*> result_directories;
        std::vector<const directory_t*> all_directories;
        all_directories.push_back(_file_system.getRootDir());

        _file_system.getRootDir()->get_all_directories(all_directories);
        for(const auto& dir : all_directories) {
            const auto dir_size = dir->getSize();
            if( dir_size <= at_most) {
                result_directories.push_back(dir);
            }
        }
    ...
    [[nodiscard]] std::size_t getSolutionPart1() const {
        auto dirs = getDirectorySmallerEqualThen(100000);

        return std::accumulate(dirs.cbegin(), dirs.cend(), 0,
                               [](std::size_t sum, const auto &dir) {
                                return sum + dir->getSize(); });
    }

    std::size_t getSolutionPart2(const std::size_t need_space_for_update) {
        std::vector<const directory_t*> all_directories;
        all_directories.push_back(_file_system.getRootDir());
        _file_system.getRootDir()->get_all_directories(all_directories);
        auto free_space = _file_system.getFreeSpace();
        auto min_to_free = need_space_for_update - free_space;

        std::size_t solution_size = 0;

        for(const auto& dir : all_directories) {
            auto dir_size = dir->getSize();
            if( dir_size <  min_to_free ) {
                continue;
            }
            if(solution_size == 0) {
                solution_size = dir_size;
                continue;
            }

            const auto sub = dir_size - min_to_free;
            if( sub < solution_size - min_to_free) {
                solution_size = dir_size;
            }
        }
...
int main() {
    ...
    std::size_t result = dev.getSolutionPart2(30000000);
