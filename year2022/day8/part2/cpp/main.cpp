...
std::size_t countVisibleTrees(long column, long row,
                              const std::vector<std::vector<int>> &forest) {

  if (column == 0 || row == 0 || column == forest[row].size() - 1 ||
      row == forest.size() - 1) {
    return 0;
  }
  ...
  return std::accumulate(
      view_in_each_direction.cbegin(), view_in_each_direction.cend(), 1,
      [](auto res, const auto &value) { return res * value; });
}
...
auto getPanoramaProduct(const std::vector<std::vector<int>> &forest) {
  ...
  for (std::size_t row = 0; row < bottom_border; row++) {
    ...
    for (std::size_t column = 0; column < right_border; column++) {
      ...
      auto trees = countVisibleTrees(static_cast<long>(column),
                                     static_cast<long>(row), forest);
