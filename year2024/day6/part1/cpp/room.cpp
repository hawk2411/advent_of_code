...
std::unique_ptr<Room> Room::Create(std::ifstream &input_data) {
  ...
  while (std::getline(input_data, text_line)) {
    ...
    for (int x = 0; x < new_room->GetWidth(); ++x) {
      if (text_line[x] == '#') {
        if (!new_room->PlaceObstruction(
                Position{x, new_room->GetLength() - 1})) {
...
void Room::SetWidth(int size) { _width = size; }

void Room::IncreaseLength() { _length++; }

int Room::GetWidth() const { return _width; }

int Room::GetLength() const { return _length; }

bool Room::IsOutOfBound(const Position &position) const {
  return (position.X() >= _width || position.Y() >= _length ||
...
bool Room::IsGuardInLoop() {
  ...
  while (next_position.has_value()) {
    ...
    if (!insert_result.second) {
      std::cout << std::format("Found loop after {0} steps",
...
Guard Room::GetGuard() const { return _guard; }
...
void Room::PlaceGuard(const Guard &guard) { _guard = guard; }
