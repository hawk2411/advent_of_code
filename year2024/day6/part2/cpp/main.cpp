...
// std::optional<position_t> get_next_obstacle_position(const
// std::vector<std::string>& room, const std::set<position_t>& obstructions,
// const guard_t& guard_position)
//{
//   int next_x = guard_position.position.first +
//   guard_position.direction.first; int next_y = guard_position.position.second
//   + guard_position.direction.second; if (next_x >= room[0].size() || next_y
//   >= room.size() || next_x < 0 || next_y < 0)
//   {
//     //out of room border
//     return {};
//   }
...
// }
//
// std::optional<position_t> do_next_step(const std::vector<std::string>& room,
// const std::set<position_t>& obstructions, const position_t& guard_position,
// direction_t& guard_direction) {
//   int next_x = guard_position.first + guard_direction.first;
//   int next_y = guard_position.second + guard_direction.second;
//   if (next_x >= room[0].size() || next_y >= room.size() || next_x < 0 ||
//   next_y < 0)
//   {
//     //out of room border
//     return {};
...
// }
//
// bool is_loop(const std::vector<std::string>& room, const
// std::set<position_t>& obstacles, const std::optional<position_t>&
// start_postion, const position_t& guard_position, const direction_t&
// direction) {
//
...
//
//   auto next_position = do_next_step(room, obstacles, guard_position,
//   virt_direction); while(next_position.has_value()) {
//     if(next_position.value() == start_postion) {
//       return true;
//     }
//     next_position = do_next_step(room, obstacles, next_position.value(),
//     virt_direction);
//   }
